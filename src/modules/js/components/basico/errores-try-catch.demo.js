...
export function init(root) {
  ...
  root.querySelector('#err-btn-referen').addEventListener('click', () => {
    try {
      // Simulamos el error sin catch
      out.innerHTML = `
        ❌ <strong>ReferenceError</strong> sin protección<br>
        Si ejecutaras este código, JavaScript se detendría:<br>
        <code>console.log(variableNoDefinida);</code><br><br>
        La página continuaría rota si esto estuviera en código real.
      `;
    } catch (e) {
  ...
  root.querySelector('#err-btn-referen-catch').addEventListener('click', () => {
    try {
      try {
        console.log(variableNoDefinida);  // ❌ Falla
      } catch (error) {
        throw error;
      }
    } catch (error) {
      out.innerHTML = `
        ✅ <strong>Error atrapado con try/catch</strong><br>
        <span style="color: red;">❌ ${error.message}</span><br><br>
        Pero el código sigue ejecutándose, ¡la página no se rompe!
      `;
    }
  });
  ...
  root.querySelector('#err-btn-type').addEventListener('click', () => {
    out.innerHTML = `
      ❌ <strong>TypeError</strong> sin protección<br>
      Si ejecutaras: <code>null.toUpperCase()</code><br><br>
      <span style="color: red;">Cannot read property 'toUpperCase' of null</span><br><br>
      La página se detendría.
    `;
  });
  ...
  root.querySelector('#err-btn-json-fail').addEventListener('click', () => {
    ...
    try {
      ...
      out.innerHTML = `
        ⚠️ <strong>JSON.parse() falló sin protección</strong><br>
        <span style="color: red;">❌ ${error.message}</span><br><br>
        Deberías SIEMPRE envolver JSON.parse() en try/catch.
      `;
    }
  });
  ...
  root.querySelector('#err-btn-json-safe').addEventListener('click', () => {
    ...
    try {
      const obj = JSON.parse(json);
      out.innerHTML = `
        ✅ <strong>JSON parseado correctamente</strong><br>
        Objeto: <code>${JSON.stringify(obj)}</code>
      `;
    } catch (error) {
      out.innerHTML = `
        ✅ <strong>Error atrapado correctamente</strong><br>
        <span style="color: orange;">⚠️ JSON inválido: ${error.message}</span><br><br>
        Pero el código sigue ejecutándose.
      `;
    }
  });
  ...
  root.querySelector('#err-btn-throw').addEventListener('click', () => {
    out.innerHTML = `
      ❌ Validación sin protección<br>
      Si la edad fuera -5, se lanzaría un error y la página se detendría.
    `;
  });
  ...
  root.querySelector('#err-btn-throw-catch').addEventListener('click', () => {
    ...
    try {
      ...
      out.innerHTML = `
        ✅ <strong>Error atrapado</strong><br>
        <span style="color: orange;">⚠️ Validación fallida: ${error.message}</span>
      `;
    }
  });
  ...
  root.querySelector('#err-btn-finally-1').addEventListener('click', () => {
    let resultado = '';
    try {
      resultado += '✅ Código en try ejecutado<br>';
      const x = 5;
      resultado += `Resultado: ${x * 2}<br>`;
    } catch (e) {
      resultado += '❌ Código en catch<br>';
    } finally {
      resultado += '<span style="color: blue;">🔵 Bloque finally SIEMPRE ejecuta</span>';
    }
  ...
  root.querySelector('#err-btn-finally-2').addEventListener('click', () => {
    let resultado = '';
    try {
      resultado += '❌ Intentando acceder variable no definida<br>';
      console.log(noExiste);
    } catch (e) {
      resultado += `⚠️ Código en catch atrapó: ${e.message}<br>`;
    } finally {
      resultado += '<span style="color: blue;">🔵 Bloque finally SIEMPRE ejecuta (incluso tras error)</span>';
    }
    out.innerHTML = resultado;
  });
}
