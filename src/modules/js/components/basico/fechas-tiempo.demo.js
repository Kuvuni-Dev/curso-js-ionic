...
export function init(root) {
  ...
  root.querySelector('#fecha-btn-ahora').addEventListener('click', () => {
    ...
    root.querySelector('#fecha-output-ahora').innerHTML = `
      <strong>Fecha y hora actual:</strong><br>
      ${formatted}<br><br>
      <code>new Date();</code> → ${ahora}
    `;
    out.innerHTML = `✅ Objeto Date creado con la fecha actual`;
  });
  ...
  root.querySelector('#fecha-btn-partes').addEventListener('click', () => {
    ...
    root.querySelector('#fecha-output-partes').innerHTML = `
      <strong>Componentes de la fecha:</strong><br>
      • getFullYear(): ${ahora.getFullYear()}<br>
      • getMonth(): ${ahora.getMonth()} (0=enero, 11=diciembre)<br>
      • getDate(): ${ahora.getDate()} (1-31)<br>
      • getDay(): ${ahora.getDay()} (0=domingo, 6=sábado)<br>
      • getHours(): ${ahora.getHours()} (0-23)<br>
      • getMinutes(): ${ahora.getMinutes()} (0-59)<br>
      • getSeconds(): ${ahora.getSeconds()} (0-59)<br>
      • getTime(): ${ahora.getTime()} (ms desde 1970)
    `;
    out.innerHTML = `✅ Se extrajeron todos los componentes de la fecha`;
  });
  ...
  root.querySelector('#fecha-btn-crear').addEventListener('click', () => {
    ...
    const fecha = new Date(year, month, 1);  // Día 1 del mes
    const formatted = fecha.toLocaleString('es-ES');
    
    root.querySelector('#fecha-output-crear').innerHTML = `
      <strong>Fecha creada:</strong><br>
      new Date(${year}, ${month}, 1)<br><br>
      Resultado: ${formatted}
    `;
    out.innerHTML = `✅ Fecha personalizada creada: ${formatted}`;
  });
  ...
  root.querySelector('#fecha-btn-diff-dias').addEventListener('click', () => {
    ...
    const nacimiento = new Date(1995, 0, 15);  // 15 de enero de 1995
    const diasPasados = Math.floor((hoy - nacimiento) / (1000 * 60 * 60 * 24));
    
    root.querySelector('#fecha-output-diff').innerHTML = `
      <strong>Diferencia entre fechas:</strong><br>
      Hoy: ${hoy.toLocaleDateString('es-ES')}<br>
      Fecha referencia: ${nacimiento.toLocaleDateString('es-ES')}<br><br>
      Diferencia: ${diasPasados} días
    `;
    out.innerHTML = `✅ Calculada diferencia: ${diasPasados} días`;
  });
  ...
  root.querySelector('#fecha-btn-diff-horas').addEventListener('click', () => {
    ...
    root.querySelector('#fecha-output-diff').innerHTML = `
      <strong>Tiempo desde medianoche:</strong><br>
      Ahora: ${ahora.toLocaleTimeString('es-ES')}<br>
      Medianoche hoy: ${hoy.toLocaleTimeString('es-ES')}<br><br>
      Transcurridas: ${horasTranscurridas}h ${minutosTranscurridos}m
    `;
    out.innerHTML = `✅ Tiempo desde medianoche: ${horasTranscurridas}h ${minutosTranscurridos}m`;
  });
  ...
  root.querySelector('#fecha-btn-perf-rapido').addEventListener('click', () => {
    ...
    root.querySelector('#fecha-output-perf').innerHTML = `
      <strong>⚡ Tarea rápida:</strong><br>
      Sumar 1 millón de números<br><br>
      Tiempo: <strong>${tiempo}ms</strong><br>
      Suma: ${suma}
    `;
  ...
  root.querySelector('#fecha-btn-perf-lento').addEventListener('click', () => {
    ...
    // Tarea más lenta: operaciones complejas
    let resultado = 0;
    for (let i = 0; i < 100000000; i++) {
      resultado = Math.sqrt(i) * Math.sin(i);
    }
    ...
    root.querySelector('#fecha-output-perf').innerHTML = `
      <strong>🐢 Tarea lenta:</strong><br>
      100 millones de operaciones (sqrt, sin)<br><br>
      Tiempo: <strong>${tiempo}ms</strong>
    `;
    out.innerHTML = `✅ Tarea completada en ${tiempo}ms`;
  });
